...
import com.nxtlife.efkon.enforcementconfigurator.dao.jpa.SequenceGeneratorJpaDao;
...
public abstract class BaseService {
	 ...
	 * @throws RuntimeException - if sequence didn't increment
	 */
	protected Integer sequenceGenerator(String type) {
		...
		int rows = sequenceGeneratorJpaDao.incrementSequence(sequenceGenerator.getId(), getUserId(), new Date());
		if (rows == 0) {
	...
	protected void updateSequenceGenerator(String type, Integer sequence) {
		...
		int rows = sequenceGeneratorJpaDao.updateSequence(sequenceGenerator.getId(), sequence, getUserId(), new Date());
		if (rows == 0) {
			throw new RuntimeException("sequence generator row updated is zero");
		}
	}
