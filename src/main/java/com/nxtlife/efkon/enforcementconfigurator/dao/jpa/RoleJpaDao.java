...
public interface RoleJpaDao extends JpaRepository<Role, Long> {
	...
	public List<RoleResponse> findByOrganizationId(Long organizationId);

	public Role findByOrganizationIdAndName(Long organizationId, String name);
	...
	@Query(value = "select id from role where organization_id=?1 and active=?2", nativeQuery = true)
	public Set<Long> findIdsByOrganizationIdAandActive(Long organizationId, Boolean active);
